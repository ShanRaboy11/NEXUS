...
namespace NEXUS.Forms
{
    public partial class Receipt : Form
    {
        ...
        public Receipt(int userID, string message)
        {
            InitializeComponent();
        ...
        public void DisplaySelectedTrip(int tripID)
        {
            string tripQuery = @"
        SELECT [Trip Date], Passenger, Driver, [Fare Amount]
        FROM Trips WHERE TripID = ?";

            using (OleDbConnection conn = DatabaseManagement.GetConnection())
            using (OleDbCommand cmd = new OleDbCommand(tripQuery, conn))
            {
                cmd.Parameters.AddWithValue("?", tripID);
                conn.Open();
